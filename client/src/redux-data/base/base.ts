...
export class CreatorReducer<T = IBaseActions, TState = IBaseState> {
  ...
  public addAction = (name: string | any,
  ...
  public createActions: () => T = () => {
    if(!this.actions) {
      this.actions = createActions(this.actionMap,
  ...
  public createReducerFetch = (initState?: any) => {
    ...
    Object.keys(this.reducerMap).forEach(key => {
      const name = rest[key];
      if(name)
        maps[name] = this.reducerMap[key];
      else
        maps[key] = this.reducerMap[key];
    });
    ...
    return handleActions<IBaseState, any>({
        ...
        [setError]: (state, action) => {
            return {...this.initState, error: action.payload}
        },
        ...maps
      },
